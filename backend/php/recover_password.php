...
try {
    ...
    $admins = executer($sql);
    if (count($admins) > 0) {
        $letters = array('6', '7', '8', '9', '0', '&', 'A', 'a', 'B', 'b', 'C', 'c', 'D', 'd', 'E', 'e', 'F', 'f', 'G', 'g', 'H', 'h', 'J', 'j', 'K', 'k', 'L', 'M', 'm', 'N', 'n', 'O', 'o', 'P', 'p', 'Q', 'q', 'R', 'r', 'S', 's', 'T', 't', 'U', 'u', 'V', 'v', 'W', 'w', 'X', 'x', 'Y', 'y', 'Z', 'z', '1', '2', '3', '4', '5');
        $password = '';
        for ($x = 0; $x <= 8; $x++) {
        ...
        */
        ...
        $mail->Body = 'You requested to change your admin password.Your new password is  ' . $password . '  .Please proceed to your profile to change password when you login.'; // Set a plain text body.
        ...
        if ($mail->send()) {
            $sql = "UPDATE admins SET admin_password='$encrypted_password' WHERE email='$email'";
                    noResultQuery($sql);
                    exit("sent");
        } else {
